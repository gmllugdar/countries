...
export function getCountries(){
  ...
  let result= await axios(`http://localhost:3001/countries`)
...
export function findCountries_N(Nombre){
  return async(dispatch)=>{ 
  let result= await axios(`http://localhost:3001/countries?name=${Nombre}`)
...
export function getCountry(id) {
 ...
 export function getActivities() {
   return async(dispatch)=>{
        var result = await axios.get(`http://localhost:3001/turismo`)
 ...
 export function postActivity(data) {
   console.log(data)
   return async ()=>{
       await axios.post(`http://localhost:3001/turismo/create`,{...data})
   }
 }
 export function findCountries_C(continent) {
  return async(dispatch)=>{
    var result = await axios.get(`http://localhost:3001/countries?continent=${continent}`)
    return dispatch({type:FIND_COUNTRIES_CONT, payload:result.data})
}
 }
 export function findCountries_A(activity) {
  return async(dispatch)=>{
    var result = await axios.get(`http://localhost:3001/turismo/paises?activity=${activity}`)
    return dispatch({type:FIND_COUNTRIES_ACT , payload:result.data})
 }
}
export function findActivity(activity) {
  return async(dispatch)=>{
    var result = await axios.get(`http://localhost:3001/turismo/nombre?nombre=${activity}`)
    return dispatch({type:FIND_ACTIVITY , payload:result.data})
 }
}
export function deleteAct(nombre) {
  return async() => {
    return await axios.delete(`http://localhost:3001/turismo/delete?nombre=${nombre}`)
  }
}